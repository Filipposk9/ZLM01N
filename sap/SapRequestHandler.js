...
class SapRequestHandler {
  ...
  static async getMaterialBasicData(matnr, charg, menge, lgortIn) {
    const credentials = JSON.parse(await CredentialStorage.getCredentials());
  ...
  static async getStorageLocations(storageLocation = 1) {
    const credentials = JSON.parse(await CredentialStorage.getCredentials());

    return await fetch(
      SapRequestHandler.baseUrl + '/mdata' + '?lgort=' + storageLocation,
      {
        method: 'GET',
        headers: {
          'Content-type': 'application/json',
